...
function quantile( p, options ) {
	...
	if ( isTypedArrayLike( p ) ) {
		...
		return quantile6( out, p, opts.v );
	}
	if ( isArrayLike( p ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return quantile4( p, opts.v, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return quantile3( out, p, opts.v, opts.accessor );
		}
		return quantile2( out, p, opts.v );
	}
	return NaN;

...
describe( 'number quantile', function tests() {
	...
	it( 'should evaluate the quantile function of the Student t distribution', function test() {
		var actual;
		for ( var i = 0; i < data.length; i++ ) {
			actual =  quantile( data[ i ], v );
	...
	it( 'should return `NaN` if provided `NaN` as input', function test() {
		assert.isTrue( isnan( quantile( NaN, v ) ) );
	});

	it( 'should return `NaN` if provided a number outside [0,1]', function test() {
		assert.isTrue( isnan( quantile( 1.1, v ) ) );
		assert.isTrue( isnan( quantile( -0.1, v ) ) );
	});
